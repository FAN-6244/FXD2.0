...
def predict_removal_effluent(input_data):
    ...
    返回: dict { 'removal': {指标: 去除率}, 'effluent': {指标: 出水浓度} }
    """
    if input_data is None:
        return None
    try:
        # 构造特征向量
        vec = np.array([input_data[col].values[0] if col in input_data.columns else 0 
        ...
        # 获取进水浓度
        cod_in = input_data['COD_load'].values[0] * 1000 / input_data['流量'].values[0] if 'COD_load' in input_data.columns else 200
        nh3_in = input_data['NH3_load'].values[0] * 1000 / input_data['流量'].values[0] if 'NH3_load' in input_data.columns else 20
        tp_in = input_data['TP_load'].values[0] * 1000 / input_data['流量'].values[0] if 'TP_load' in input_data.columns else 3
        tn_in = input_data.get('TN', [30])[0] if 'TN' in input_data.columns else 30
        ss_in = input_data.get('SS', [150])[0] if 'SS' in input_data.columns else 150
        
        # 去除率（NH3-N 用模型预测，其他用经验值）
        removal_rates = {
            'NH3-N': pred_removal,
            'COD': 0.93,
            'TP': 0.88,
...
if input_data is not None:
    ...
    if pred_result is not None:
        ...
        removal_rates = pred_result['removal']
        ...
        with col_ch2:
            ...
            <div class="channel-item channel-special">
                ...
                <div class="ch-desc">COD — 去除冗余，不适用记忆长度</div>
